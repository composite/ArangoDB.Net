...
namespace ArangoDB.Velocypack.Internal
{
    ...
    public class Value
    {
        ...
        {
            if (type != ValueType.ARRAY && type != ValueType
                .OBJECT && type != ValueType.NULL)
            {
        ...
        /// <exception cref="VPackValueTypeException"/>
        public Value(long value, ValueType type) : this(value, type, typeof(long))
        {
            if (type != ValueType.INT && type != ValueType
                .UINT && type != ValueType.SMALLINT)
            {
                throw new VPackValueTypeException(ValueType
        ...
        /// <exception cref="VPackValueTypeException"/>
        public Value(decimal value, ValueType type) : this(value, type, typeof(decimal))
        {
            if (type != ValueType.INT && type != ValueType
                .UINT && type != ValueType.SMALLINT)
            {
